...
function myAutoloadClasses($className) {
	...
	// echo $className;
	$filePath = $sourcedir . '/LikePosts/' . $className . '.php';
...
function addActionContext(&$actions) {
	global $context, $sourcedir;

	$context['like_posts'] = array(
		'source_dir' => $sourcedir . '/LikePosts/'
	);

	// $actions['likeposts'] = array(LikePosts::$sourceFolder . 'LikePostsDispatcher.php', 'LikePostsDispatcher::mainIndex');
	// $actions['likepostsstats'] = array(LikePosts::$sourceFolder . 'LikePostsDispatcher.php', 'LikePostsDispatcher::statsIndex');
}
...
function addMenuItems(&$menu_buttons) {
	...
	if (!self::$lpUtils->isAllowedTo(array('can_view_likes_stats'))) {
		$isAllowedToAccess = false;
	}
...
function addAdminPanel(&$admin_areas) {
	global $txt;

	$admin_areas['config']['areas']['likeposts'] = array(
		'label' => $txt['lp_menu'],
		'file' => LikePosts::$sourceFolder . 'LikePostsAdmin.php',
...
function addProfilePanel(&$profile_areas) {
